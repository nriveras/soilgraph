...
#'
#' `plot_soil_description_fragments()` is deprecated because it relies on the
#' free-text parser. Build a `soil_profile` from a structured table with
#' [soil_profile_from_table()] and pass it to [plot_soil_profile_fragments()] or
#' [plot_soil_profile_advanced()] instead.
#'
...
plot_soil_description_fragments <- function(
    ...
    seed = 1) {
  .Deprecated(
    "plot_soil_profile_fragments",
    package = "soilgraph",
    msg = paste(
      "`plot_soil_description_fragments()` is deprecated.",
      "Use `soil_profile_from_table()` with a structured data frame,",
      "then pass the result to `plot_soil_profile_fragments()` or",
      "`plot_soil_profile_advanced()`."
    )
  )

  profile <- suppressWarnings(soil_profile_from_table(
    data = data,
    site_id = site_id,
...
#' `r lifecycle::badge("deprecated")`
#'
#' `plot_soil_description()` is deprecated because it relies on the free-text
#' parser. Build a `soil_profile` from a structured table with
#' [soil_profile_from_table()] and pass it to [plot_soil_profile()] or
#' [plot_soil_profile_advanced()] instead.
#'
#' @param data A data frame with `Depth` and `description` columns.
#' @param site_id Profile identifier. Defaults to `"soil-profile"`.
...
plot_soil_description <- function(
  ...
  .Deprecated(
    ...
    package = "soilgraph",
    msg = paste(
      "`plot_soil_description()` is deprecated.",
      "Use `soil_profile_from_table()` with a structured data frame,",
      "then pass the result to `plot_soil_profile()` or",
      "`plot_soil_profile_advanced()`."
    )
  )
